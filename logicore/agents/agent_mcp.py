...
class MCPAgent:
    ...
    def __init__(
        ...
        self.simplemem = AgentrySimpleMem(user_id="mcp_user", session_id="mcp_session") if memory else None
    ...
    async def chat(
        ...
        if self.memory_enabled and self.simplemem:
            context = await self.simplemem.on_user_message(user_input)
            if context:
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                on_token = streaming_funct or getattr(self, 'on_token', None)
                has_stream = hasattr(self.provider, 'chat_stream')
                
                if stream and on_token and has_stream:
                    response_message = await self.provider.chat_stream(session.messages, tools=all_tools, on_token=on_token)
                else:
            ...
            if not tool_calls:
                if self.telemetry_enabled:
                    est_in = len(str(session.messages)) // 4
                    est_out = len(str(content)) // 4 if content else 0
                    self.telemetry_tracker.record_request("mcp_model", time.time() - start_time, est_in, est_out, 0)
                if self.memory_enabled and self.simplemem and content:
            ...
            for tool_call in tool_calls:
                ...
                if self.telemetry_enabled:
                    if not hasattr(self.telemetry_tracker, 'current_session'):
                        self.telemetry_tracker.start_session(session_id)
                    self.telemetry_tracker.current_session.tool_calls += 1
