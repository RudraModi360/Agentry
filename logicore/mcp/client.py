...
class MCPClientManager:
    ...
    async def load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"{label('MCP Client', RED)} Error loading config: {e}")
            return {}
    ...
    async def _run_server_connection(
        ...
        try:
            ...
            is_stopping = (
                server_name in self.server_stop_events
                and self.server_stop_events[server_name].is_set()
            )
            if not is_stopping:
                print(
                    f"{label('MCP Client', YELLOW)} Connection error for {server_name}: {e}"
                )
    ...
    async def connect_to_servers(self):
        ...
        for server_name, server_config in servers.items():
            # Skip if it's the logicore server itself to avoid recursion
            if server_name.startswith("logicore"):
                continue
            ...
            try:
                ...
                server_params = StdioServerParameters(
                    command=cmd_path, args=args, env=full_env
                )
                ...
                task = asyncio.create_task(
                    self._run_server_connection(server_name, server_params, ready_event)
                )
                ...
                try:
                    ...
                    if server_name in self.sessions:
                        print(
                            f"{label('MCP Client', GREEN)} Connected to server: {success(server_name)}"
                        )

                        # List tools and map them
                        result = await self.sessions[server_name].list_tools()
                        print(
                            f"{label('MCP Client', GREEN)} Found {success(str(len(result.tools)))} tools from {server_name}"
                        )
                        for tool in result.tools:
                            self.server_tools_map[tool.name] = server_name
                    else:
                        print(
                            f"{label('MCP Client', RED)} Failed to connect to {server_name} (Initialization failed)"
                        )

                except asyncio.TimeoutError:
                    print(
                        f"{label('MCP Client', YELLOW)} Timeout connecting to {server_name}"
                    )
                    # Don't kill the task, it might just be slow, but we can't wait forever

            except Exception as e:
                print(
                    f"{label('MCP Client', RED)} Failed to setup {server_name}: {e}"
                )
        ...
        for server_name, session in self.sessions.items():
            try:
                ...
                for tool in result.tools:
                    ...
                    all_tools.append(logicore_tool)
            except Exception as e:
                print(
                    f"{label('MCP Client', RED)} Error listing tools from {server_name}: {e}"
                )
