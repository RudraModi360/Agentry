...
try:
    import logicore
    if not hasattr(logicore, '__file__') or logicore.__file__ is None:
        import importlib
        # Ensure our local path is definitely in sys.path and AT THE FRONT
        if parent_dir in sys.path:
            sys.path.remove(parent_dir)
        sys.path.insert(0, parent_dir)
        importlib.reload(logicore)
except Exception:
    pass

from logicore.agents import Agent
from logicore.agents import SmartAgent, SmartAgentMode, CopilotAgent
from logicore.config.settings import get_api_key
from logicore.session_manager import SessionManager
from logicore.reloader import start_reloader
from logicore.client.app import ClientApp
from logicore.client.display import DisplayManager
...
logicore -a smart -m llama3.2:3b

[green]# Smart Agent in Project Mode[/green]
logicore -a smart --mode project --project my-app

[green]# Use Groq provider with specific model[/green]
logicore -a smart -p groq -m llama-3.3-70b-versatile

[green]# Use Gemini[/green]
logicore -p gemini -m gemini-2.0-flash

[green]# Copilot for coding[/green]
logicore -c -p groq -m llama-3.3-70b-versatile

[green]# List available Ollama models[/green]
logicore --list-models
"""
...
def list_models():
    ...
    try:
        ...
        if models_list.get('models'):
            ...
            console.print(table)
            console.print(f"\n[dim]Use with: logicore -m <model_name>[/dim]")
        else:
    ...
    console.print(f"\n[dim]Use with: logicore -p groq -m <model_name>[/dim]")
    ...
    console.print(f"\n[dim]Use with: logicore -p gemini -m <model_name>[/dim]\n")
    ...
    console.print(f"\n[dim]Use with: logicore -p azure --endpoint <url> -m <deployment_name>[/dim]\n")
...
async def run_main():
    ...
    with console.status("[bold green]Booting logicore...[/]", spinner="dots") as status:
        ...
        import logicore
        watch_dir = os.path.dirname(os.path.abspath(logicore.__file__))
